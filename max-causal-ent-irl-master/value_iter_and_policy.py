...
def torch_qi(mdp, gamma, r, horizon=None,  temperature=1, 
                            threshold=1e-16, use_mellowmax=False):
    '''
    '''
    #Value iteration    
    V = torch.zeros(mdp.nS).double()#torch.clone(r)
    t = 0
    diff = float("inf")
    T = torch.tensor(mdp.T).double()
    while diff > threshold:
        V_prev = torch.clone(V)
        
        # ∀ s,a: Q[s,a] = (r_s + gamma * \sum_{s'} p(s'|s,a)V_{s'})
        Q = r + gamma * torch.matmul(T, V_prev)
        if use_mellowmax:
            ...
            #V = tsoftmax(Q, temperature)
            V = torch.log( torch.sum( torch.exp( Q ), dim = 1 ) ) #torch.nn.functional.softmax(Q/ temperature, dim = 1).amax(dim = 1)
            

        diff = torch.amax(abs(V_prev - V))
...
def torch_boltzmann(mdp, gamma, r, horizon=None,  temperature=1, 
                            threshold=1e-16, use_mellowmax=False):
    '''
    '''
    #Value iteration    
    ...
    t = 0
    diff = float("inf")
    T = torch.tensor(mdp.T)
    while diff > threshold:
        V_prev = torch.clone(V)
        
        # ∀ s,a: Q[s,a] = (r_s + gamma * \sum_{s'} p(s'|s,a)V_{s'})
        Q = r.reshape((-1,1)) + gamma * torch.matmul(T, V_prev)
        if use_mellowmax:
            # ∀ s: V_s = temperature * log(\sum_a exp(Q_{sa}/temperature) / nA)
            V = mellowmax(Q, temperature)
        else:
...
def qi_boltzmann(mdp, gamma, r, horizon=None,  temperature=1, 
                            threshold=1e-16, use_mellowmax=False):
    #Value iteration    
    V = np.zeros(mdp.nS) #np.copy(r)
    t = 0
    diff = float("inf")
    while diff > threshold:
        V_prev = np.copy(V)
...
def tsoftmax(x, t=1):
    ...
    if t == 0: return torch.amax(x, axis=1)
    ...
    def softmax_2_arg(x1,x2, t):
        ...
        '''
        tlog = lambda x: t * torch.log(x)
        expt = lambda x: torch.exp(x/t)
        
        
                
        max_x = torch.max(torch.tensor(x1,x2),axis=0)
        min_x = torch.max(torch.tensor(x1,x2),axis=0)    
